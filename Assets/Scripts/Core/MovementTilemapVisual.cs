...
namespace OperationBlackwell.Core {
	public class MovementTilemapVisual : MonoBehaviour {
		...
		private void UpdateHeatMapVisual() {
			MeshUtils.CreateEmptyMeshArrays(grid_.GetWidth() * grid_.GetHeight(), out Vector3[] vertices, out Vector2[] uv, out int[] triangles);
			...
			for (int x = 0; x < grid_.GetWidth(); x++) {
				for (int y = 0; y < grid_.GetHeight(); y++) {
					...
					MeshUtils.AddToMeshArrays(vertices, uv, triangles, index, grid_.GetWorldPosition(x, y) + quadSize * .5f, 0f, quadSize, gridUV00, gridUV11);
				}
			}
