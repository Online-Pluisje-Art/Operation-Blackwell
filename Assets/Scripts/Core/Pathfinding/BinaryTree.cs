...
namespace OperationBlackwell.Core {
	public class BinaryTree {
		...
		public void AddNode(PathNode pathNode) {
			//debugCodePath += "AD; ";
			if(root_ == null) {
				root_ = new BinaryNode(pathNode, null, null);
				totalNodes++;
			} else {
				// Has root_
				//debugString = "";
				try {
					addNode(root_, pathNode, pathNode.fValue);
		...
		public void RemoveNode(PathNode pathNode) {
			if(root_.pathNode == pathNode) {
				// It's the root_
				//debugCodePath += "RR" + root_ + "; ";
				BinaryNode prevRoot = root_;
				if(root_.left == null && root_.right == null) {
					...
					if(root_.left == null && root_.right != null) {
						...
						if(root_.right == null && root_.left != null) {
							...
							if(root_.left.right == null) {
								// Root left has no right
								root_.left.right = root_.right;
								root_ = root_.left;
							} else {
								// Root left has a right
								BinaryNode leafRight = getLeafRight(root_.left);
								root_ = leafRight.right;
								if(leafRight.right.left != null) {
									// This leaf has a left
									leafRight.right = leafRight.right.left;
								}
								leafRight.right = null;
								root_.left = leafRight;
								root_.right = prevRoot.right;
							}
						}
					}
				}
				...
				removeNode(root_, pathNode, pathFvalue);
			}
			totalNodes--;
		}
		...
		public PathNode GetSmallest() {
			...
			int count = 0;
			try {
				pathNode = getSmallest(root_, ref count);
			} catch {
				Debug.LogError("GetSmallest: " + count);
			}
			return pathNode;
			//return getSmallest(root_, 0);
		}
